...
public class VerifyGatewayFilter implements GlobalFilter, Ordered {
  ...
  private void logRequest(ServerHttpRequest request, String body) {
    ...
    headers.forEach((key, value) -> log.info("Request -> headers: key->{}, value->{}", key, value));
    MultiValueMap<String, String> queryParams = request.getQueryParams();
    if (!queryParams.isEmpty()) {
      queryParams.forEach(
          (key, value) -> log.info("Request -> query param: key->{}, value->{}", key, value));
    }
    if (body != null) {
  ...
  private ServerHttpResponseDecorator logResponse(ServerWebExchange exchange) {
    ...
    Optional<Long> startTime = Optional.ofNullable(exchange.getAttribute(START_TIME));
    HttpHeaders headers = origResponse.getHeaders();
    headers.forEach(
        (key, value) -> log.info("Response -> headers: key->{}, value->{}", key, value));
    Long executeTime = (System.currentTimeMillis() - startTime.orElse(0L));
    log.info(
